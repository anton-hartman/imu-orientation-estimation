...
def read_sensor_data():
    ...
    folderName = os.path.join(folderName, "Collected_Data")
    ...
    for monteNumber in range(1, numFiles + 1):
        ...
        SENSOR_READINGS[monteNumber - 1] = np.array(
...
def plot_true_estimated(orientation, baseline, research):
    ...
    plt.text(
        0.5, 0.04, "Time (s)", ha="center", va="center", transform=plt.gcf().transFigure
    )
    plt.text(
        0.06,
        0.5,
...
def show_optimal_test_table(intervals):
    ...
    for interval in intervals:
        ...
        baseline_state = ESTIMATED_STATE_baseline[
            :, interval[0] * Fs : interval[1] * Fs
        ]
        research_state = ESTIMATED_STATE_research[
            :, interval[0] * Fs : interval[1] * Fs
        ]
    ...
    # print header
    print(
        "{:>15}       {:^30}      {:^30}      {:^30}".format("", "Pitch", "Roll", "Yaw")
    )
    head_str = "{:<10}{:<10}{:<10}".format("mean", "std", "T")
    print(
        "{:>15}       {:<30}      {:<30}      {:<30}".format(
            "Time interval", head_str, head_str, head_str
        )
    )

    for result, interval in zip(results, intervals):
        p_str = "{:<+10.2f}{:<+10.2f}{:<+10.2f}".format(
            result[0][0], result[1][0], result[2][0]
        )
        r_str = "{:<+10.2f}{:<+10.2f}{:<+10.2f}".format(
            result[0][1], result[1][1], result[2][1]
        )
        y_str = "{:<+10.2f}{:<+10.2f}{:<+10.2f}".format(
            result[0][2], result[1][2], result[2][2]
        )
        print(
            "{:>15}       {:<30}      {:<30}      {:<30}".format(
                str(interval), p_str, r_str, y_str
            )
        )
...
estimated_state_baseline = get_estimated_state(
    gyro_readings, acc_readings, mag_readings
)

FILTER = FILTER_RESEARCH

estimated_state_research = get_estimated_state(
    gyro_readings, acc_readings, mag_readings
)
...
for n in range(N):
    ...
    true_state = [
        Quaternion(Quaternion().from_angles(ang * DEG2RAD)) for ang in true_state
    ]
