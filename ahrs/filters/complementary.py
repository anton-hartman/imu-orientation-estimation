...
class Complementary:
    ...
    def _compute_all(self) -> np.ndarray:
        ...
        Q[0] = (
            self.am_estimation(self.acc[0], self.mag[0])
            if self.q0 is None
            else self.q0.copy()
        )
    ...
    def am_estimation(self, acc: np.ndarray, mag: np.ndarray = None) -> np.ndarray:
        ...
        if acc.shape[-1] != 3:
            raise ValueError(
                f"Accelerometer sample must have three elements. It has {acc.shape[-1]}"
            )
    ...
    def update(
        self, q: np.ndarray, gyr: np.ndarray, acc: np.ndarray, mag: np.ndarray = None
    ) -> np.ndarray:
        """
